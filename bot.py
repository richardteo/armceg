...
route_dict = {'1': "Stadium", '2': "1km", '3': "2km", '4': "3.2km"}
location_dict = {'1': "Stadium", '2': "Parade Square", '3': "Buaya Square", '4': "Training Sheds"}
...
def check_health(update: Update, _: CallbackContext):
    ...
    if update.message.text == 'CEC':
        ...
        'Get Active Questionnaire taken from sportsingapore.gov.sg and Pre-Activity Checklist from SFT Training Directive')
        update.message.reply_text(
...
def correct_format(update: Update, _: CallbackContext):
    ...
    update.message.reply_text(
        ...
        'For Name: Name \n\n'
        'For Activity: 1/2/3 (Choose one) \n\n '
        'For Location/Route: 1/2/3/4 (Choose one) \n\n'
...
def main() -> None:
    ...
    start_conv_handler = ConversationHandler(
        ...
        states = {
            ...
            ROUTECONFIRMATION: [MessageHandler(Filters.regex('^1$|^2$|^3$|^4$'), route_confirmation)],
            LOCATIONCONF: [MessageHandler(Filters.regex('^1$|^2$|^3$|^4$'), location)],
            CONFIRMATION: [MessageHandler(Filters.regex('^1$|^2$|^3$|^4$'), confirmation)],
